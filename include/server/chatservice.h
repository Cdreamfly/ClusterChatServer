...
public:
   ...
   // void groupChat(const muduo::net::TcpConnectionPtr&conn,json&js, muduo::Timestamp timestamp);
    ...
    //获取消息id对应的处理函数
    MsgHandler GetHandler(const EnMsgType msgId);
    //处理客户端异常推出
...
private:
    ...
    std::mutex _mtx;
    UserModel::ptr _userModel;   //用户业务
    OfflineMsgModel::ptr _offlineMsgModel;   //离线消息业务
    FriendModel::ptr _friendModel;   //好友业务
    GroupModel::ptr _groupModel; //群业务
    Redis::ptr _redis;
};
